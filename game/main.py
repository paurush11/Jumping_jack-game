...
GROUNDY = SCREENHEIGHT * 0.89
GAME_SPRITES = {}
GAME_SOUNDS = {}
PLAYER = 'gallery/images/birdw.png'
BACKGROUND = 'gallery/images/nayabg.png'
...
def welcomeScreen():
    ...
    while True:
        for event in pygame.event.get():
            ...
            if event.type == QUIT or (event.type==KEYDOWN and event.key == K_ESCAPE): #pylint: disable=C0326
                pygame.quit()
                sys.exit()

            # If the user presses space or up key, start the game for them
            elif event.type==KEYDOWN and (event.key==K_SPACE or event.key == K_UP): #pylint: disable=C0326
                return
            else:
                SCREEN.blit(GAME_SPRITES['background'], (0, 0))    #pylint: disable=C0326
                SCREEN.blit(GAME_SPRITES['player'], (playerx, playery))    #pylint: disable=C0326
                SCREEN.blit(GAME_SPRITES['message'], (messagex,messagey ))    #pylint: disable=C0326
                SCREEN.blit(GAME_SPRITES['base'], (basex, GROUNDY))    #pylint: disable=C0326
...
def mainGame():
    ...
    while True:
        ...
        for pipe in upperPipes:
            ...
            if pipeMidPos<= playerMidPos < pipeMidPos +4: #pylint: disable=C0326
                score +=1
                print(f"Your score is {score}") 
                GAME_SOUNDS['point'].play()
                highscores.append([count, score])
...
if __name__ == "__main__":
    ...
    GAME_SPRITES['message'] =pygame.image.load('gallery/images/newmsg.png').convert_alpha()
    GAME_SPRITES['base'] =pygame.image.load('gallery/images/basewa.png').convert_alpha()
